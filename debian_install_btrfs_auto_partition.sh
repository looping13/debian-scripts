...
mount -o $mount_options_btrfs,subvol=@cache $dev_target /target/var/cache
...
echo "UUID=$disk_uuid /var/cache btrfs $mount_options_btrfs,subvol=@cache 0 0" >> /target/etc/fstab
